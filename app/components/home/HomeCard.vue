...
const badgeName = computed(() => {
  ...
  if (canUnlockSoundsterBadge.value) badges.push('Soundster')
...
const canUnlockADiscordBadge = computed(() => {
  return canUnlockNuxterBadge.value || canUnlockSoundsterBadge.value || canUnlockUIProBadge.value
})
...
function unlockButton(e: MouseEvent) {
  ...
  if (linked.value.discord && canUnlockSoundsterBadge) {
    popConfetti()
  }
  if (linked.value.discord && canUnlockUIProBadge) {
    popConfetti()
  }
}
onMounted(() => {
      ...
      <div v-else-if="linked.github" class="w-full h-full">
        ...
        <div
          ...
          <div class="flex flex-col gap-y-4 justify-center w-full">
            <UAvatar :src="contributor.username" size="2xl" :alt="contributor.username" class="z-10" />
